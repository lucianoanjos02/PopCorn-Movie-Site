...
try:
    '''
    Tento criar uma ENGINE de conexão com os parâmetros do config, INCLUINDO a BASE MovieApp
    e tento conectar no banco.
    '''
    engine = db.create_engine(f'{config.MYSQL_ENGINE}://{config.MYSQL_USER}:{config.MYSQL_PASSWORD}@{config.MYSQL_HOST}:{config.MYSQL_PORT}/{config.MYSQL_DB}', echo=True)
    connection = engine.connect()
except InternalError:
    '''
    Caso ocorra um InternalError devido à base MovieApp não existir: 
    
    - Crio uma nova ENGINE sem a base;
    - Abro uma conexão através da ENGINE;
    - Crio a base ChatApp dentro dessa conexão;
    - Sobreescrevo a ENGINE anterior, agora com a base MovieApp;
    - Crio uma nova conexão a ENGINE com o banco.
    '''
    engine = db.create_engine(f'{config.MYSQL_ENGINE}://{config.MYSQL_USER}:{config.MYSQL_PASSWORD}@{config.MYSQL_HOST}:{config.MYSQL_PORT}', echo=True)
    connection = engine.connect()
    connection.execute('CREATE DATABASE MovieApp;')
    ...
    Base.metadata.create_all(engine) # Cria as tabelas na base MovieApp de acordo com as classes mapeadoras.
